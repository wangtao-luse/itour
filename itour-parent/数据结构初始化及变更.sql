...
BRIEF	VARCHAR(128) COMMENT'简介',	
PLAN	TEXT COMMENT'旅行安排',		
TYPE	VARCHAR(2) COMMENT'类型(1:旅行攻略；2.周末攻略3.景点介绍;(来源于博客类型表)',
`COLUMN`	INT	COMMENT'所属专栏	来源旅行博客专栏表的ID',
...
--4.旅行博客关键字表(T_T_KEYWORD)
CREATE TABLE IF NOT EXISTS T_T_KEYWORD(
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',	
Keyword VARCHAR(18) COMMENT'关键字	博客关键字'
);
ALTER TABLE T_T_KEYWORD COMMENT '旅行博客关键字表';
--5.旅行博客-关键字表(T_TRAVEL_KEYWORD)中间表
...
ALTER TABLE T_TRAVEL_KEYWORD COMMENT '旅行博客-关键字表(中间表)';
...
DISTRICTESC	VARCHAR(500)COMMENT'行政区域划分介绍',
