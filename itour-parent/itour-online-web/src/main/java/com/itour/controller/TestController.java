...
public class TestController {
	...
	@ResponseBody
	public ResponseMessage testRedis() {
		boolean set = redisManager.set("name", "wangtao");
		boolean set2 = redisManager.set("hobby", "travel", 60);
		String string = redisManager.get("hobby");

		Map map = new HashMap<String, Object>();
		Student student = new Student("01", "mike", "10");
		map.put("student", student);
		boolean hmset = redisManager.hmset("stu", map);
		Student student1 =(Student) redisManager.hget("stu", "student");
		System.out.println(set);
		System.out.println(string);
		System.out.println(hmset);
		System.out.println(student1);
		return ResponseMessage.getSucess(string);
