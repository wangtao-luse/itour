...
public ResponseMessage uploadPicture(MultipartFile file,HttpServletRequest request) {
	...
	try {
		...
		responseMessage = FileUploadHelper.upload(file, uploadFileLocation, resourceHandler, request, FileUploadHelper.FILESIZE_1024MB);
		...
		String savePath =uploadFileLocation+File.separator+FileUploadHelper.getPath()+File.separator+fileName;
		ThumbnailsHelper.thumbnail(path, savePath, ThumbnailsHelper.SCALE_DEFUALT);
		String basePath = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort()+ request.getContextPath();
		String fileServerPath = basePath + resourceHandler.substring(0, resourceHandler.lastIndexOf("/"))+ FileUploadHelper.getPath() + File.separator + fileName;
