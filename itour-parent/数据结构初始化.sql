...
#4.旅行信息-专栏表(T_T_TRAVELINFO_COLUMN)
CREATE TABLE IF NOT EXISTS T_T_TRAVELINFO_COLUMN(
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',	
TID	INT	COMMENT'旅行信息编号',
CID INT COMMENT'专栏编号'
) COMMENT '旅行信息-专栏表';	
...
#7.3敏感字表
CREATE TABLE IF NOT EXISTS T_D_SENSITIVE_WORD(
ID INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
WORD_CONTENT VARCHAR(64)
) COMMENT '敏感字表';
#8.旅行博客收藏表(T_T_COLLECT)
CREATE TABLE IF NOT EXISTS T_T_COLLECT(
ID	INT	PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
...
#-8.1旅行收藏夹(T_T_FAVORITES)
CREATE TABLE IF NOT EXISTS T_T_FAVORITES(
ID	INT	PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
...
visual VARCHAR(2) DEFAULT '1' COMMENT '是否公开(0:紧自己可见;1:公开)',
DELETE_DATE BIGINT COMMENT'删除时间'
) COMMENT '旅行收藏夹表';
...
ROLE_NAME	VARCHAR(30) COMMENT'角色名称',	
ROLE_DESC	VARCHAR(50)COMMENT'角色描述',
CREATEDATE  BIGINT  COMMENT'角色创建时间'
);
...
CREATE TABLE IF NOT EXISTS T_T_HISTORY(
ID	INT	PRIMARY KEY AUTO_INCREMENT COMMENT '编号',
...
#------------------个人博客模块--------------------------------------------------------------------------------------------------------------------------------------
#1.个人博客信息表(T_W_WORK_INFO)
CREATE TABLE IF NOT EXISTS T_W_WORK_INFO(
ID	INT	PRIMARY KEY AUTO_INCREMENT COMMENT '编号',
TITLE	VARCHAR(50)	COMMENT'标题',
URL	VARCHAR(300)	COMMENT'博客封面	视频或图片地址',
SUMMARY	VARCHAR(128)	COMMENT'简介',	
ARTICLE_TYPE	VARCHAR(2)	COMMENT'类型	(1：原创；0非原创)',
PUBLISHTIME	BIGINT	COMMENT'发布日期',	
UPDATETIME	BIGINT	COMMENT'最后修改日期',	
MODALITY	INT	COMMENT'发布形式	(1:仅自己可见,2:公开)',
UID	VARCHAR(10) COMMENT'用户唯一号',	
READ_COUNT	INT	DEFAULT '0' COMMENT'阅读数	默认0 冗余字段',
COMMENT_COUNT	INT	DEFAULT '0'COMMENT'评论数	默认0冗余字段',
LIKE_COUNT	INT DEFAULT '0'	COMMENT'点赞数	默认0冗余字段',
PV	INT	DEFAULT '0'	COMMENT'浏览量 默认0冗余字段',
COLLECT_COUNT INT DEFAULT '0' COMMENT'收藏数',
STATUS	VARCHAR(2)	COMMENT'状态(10：草稿；20：待审核;30:审核通过；40：审核不通过；50：已删除)'
) COMMENT '个人博客信息表';
#1.1 个人博客文本(T_W_WORKTEXT)
CREATE TABLE IF NOT EXISTS T_W_WORKTEXT(
ID	INT	PRIMARY KEY AUTO_INCREMENT COMMENT '编号',
WID	INT	COMMENT '博文编号',
WCONTENT	LONGTEXT	COMMENT '工作博客文本'
) COMMENT '个人博客文本';

#2.个人博客栏目表(T_W_WORK_COLUMN)
...
#2.1个人博客信息-栏目表(T_W_INFO_COLUMN)
CREATE TABLE IF NOT EXISTS T_W_INFO_COLUMN (
ID	INT	PRIMARY KEY AUTO_INCREMENT COMMENT '编号',
WID		INT	COMMENT '博客编号',
CID		INT	COMMENT '专栏ID'
) COMMENT '个人博客信息-栏目表';
#3.个人博客标签表(T_W_lABEL)
CREATE TABLE IF NOT EXISTS T_W_lABEL(
ID	INT	PRIMARY KEY AUTO_INCREMENT COMMENT '编号',
UID	VARCHAR(10)	 COMMENT '标签创建者唯一号',
TAG	VARCHAR(20)	 COMMENT '标签名称',
CREATEDATE	BIGINT	 COMMENT '创建日期'
) COMMENT'个人博客标签表';
#3.1个人博客信息-标签表(T_W_INFO_COLUMN)
CREATE TABLE IF NOT EXISTS T_W_INFO_LABEL (
ID	INT	PRIMARY KEY AUTO_INCREMENT COMMENT '编号',
WID	INT	COMMENT '博客编号',
LABEL_ID INT	COMMENT '标签编号'
)COMMENT'个人博客信息-标签表';
#4.个人博客评论表(T_W_WORK_COMMENT)
CREATE TABLE IF NOT EXISTS T_W_WORK_COMMENT(
ID	INT	PRIMARY KEY AUTO_INCREMENT COMMENT '编号',
COMMENT	VARCHAR(1200)	COMMENT '评论内容',
CTIME	BIGINT	COMMENT '评论时间',	
UID	VARCHAR(10)	COMMENT '用户编号	用户ID',
TID	INT	COMMENT '文章信息编号	博客的编号',
NICE_COUNT	INT	 default 0 COMMENT '评论点赞数',	
STATUS	VARCHAR(3)	COMMENT '状态(20:待审核状态;30:审核通过;40:审核不通过; 50:已删除)'
) COMMENT '个人博客评论表';
#5.评论回复表(T_W_WORK_COMMENT_REPLY)
CREATE TABLE IF NOT EXISTS T_W_WORK_COMMENT_REPLY(
ID	INT	PRIMARY KEY AUTO_INCREMENT COMMENT '编号',
COMMENT_ID	INT	COMMENT '评论的ID	',
REPLY	VARCHAR(1200)	COMMENT '回复的内容	',
RTIME	BIGINT	COMMENT '回复时间	',
FROM_UID	VARCHAR(10)	COMMENT '回复人的ID	',
TO_UID	VARCHAR(10)	COMMENT '回复给目标人的ID	',
NICE_COUNT	INT default 0	COMMENT '评论点赞数	',
...
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
VIDEO_NAME	VARCHAR(80)	 COMMENT'视频名称',
VIDEO_FORMER_NAME	VARCHAR(80)	 COMMENT'视频原名',
URL	VARCHAR(300)	 COMMENT'封面',
SUMMARY	VARCHAR(1200)	 COMMENT'简介',
CN_PREMIERE_DATE	BIGINT	 COMMENT'大陆首映日期',
CRATE_DATE	BIGINT	 COMMENT'创建日期',
UID	VARCHAR(18)	 COMMENT'用户编号',
FAVOR_COUNT	INT	DEFAULT 0 COMMENT'喜欢数',
COMMENT_COUNT	INT	DEFAULT 0 COMMENT'评论数',
AREA	INT	 COMMENT'地区'
)COMMENT '视频信息';
#7.2演员表(T_V_PERFORMER_INFO)
CREATE TABLE IF NOT EXISTS T_V_PERFORMER_INFO (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
NAME	VARCHAR(80)	 COMMENT'姓名',
FORMER_NAME	VARCHAR(80)	 COMMENT'原名',
SEX	VARCHAR(2)	 COMMENT'性别',
BIRTHDAY	BIGINT	 COMMENT'生日',
NATIONALITY	VARCHAR(80)	 COMMENT'国籍',
SUMMARY	VARCHAR(1200)	 COMMENT'简介'
)COMMENT '演员表';
#7.3视频-演员表（T_V_VIDEO_PERFORMER）
CREATE TABLE IF NOT EXISTS T_V_VIDEO_PERFORMER (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
VID	INT	COMMENT'视频编号',
PID	INT	COMMENT'演员编号',
WEIGHT	DOUBLE(3,2)	COMMENT'权重'
)COMMENT '视频-演员表';
#7.4视频类型(T_V_MOLD)
CREATE TABLE IF NOT EXISTS T_V_MOLD (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
MOLD	VARCHAR(80)	COMMENT'视频类型',
CREATE_DATE	BIGINT	COMMENT'创建日期'
)COMMENT '视频类型';
#7.5视频信息-视频类型表（T_V_VIDEO_INFO_MOLD）
CREATE TABLE IF NOT EXISTS T_V_VIDEO_INFO_MOLD (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
VID	INT	COMMENT'类型编号',
MID	INT	COMMENT'类型编号'
)COMMENT '视频信息-视频类型表';
#7.6视频-视频喜欢表(T_V_VIDEO_FAVOR)
CREATE TABLE IF NOT EXISTS T_V_VIDEO_FAVOR (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
VID	INT	COMMENT'视频编号',
UID	VARCHAR(18)	COMMENT'用户编号',
CREATE_DATE	BIGINT	COMMENT'操作日期',
STATUS	VARCHAR(2)	COMMENT'状态'
)COMMENT '视频-视频喜欢表';
#7.7视频评论表(T_V_VIDEO_COMMENT)
CREATE TABLE IF NOT EXISTS T_V_VIDEO_COMMENT (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
COMMENT	VARCHAR(1200)	COMMENT'评论内容',	
CTIME	BIGINT	COMMENT'评论时间',	
UID	VARCHAR(10)	COMMENT'用户编号',
...
#7.8 视频信息-评论点赞表（T_V_VIDO_NICE）
CREATE TABLE IF NOT EXISTS T_V_VIDO_NICE (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
VID	INT	COMMENT'电影编号',
UID	VARCHAR(18)	COMMENT'用户编号',	
CREATE_DATE	BIGINT	COMMENT'操作日期',	
STATUS	VARCHAR(2)	COMMENT'状态(1：点赞;0取消点赞;)'	

)COMMENT '视频信息-评论点赞表';
#7.9视频信息-评论收集（T_V_VIDEO_COMMENT_COLLECT）
CREATE TABLE IF NOT EXISTS T_V_VIDEO_COMMENT_COLLECT (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
FID	INT	COMMENT'电影编号',	
COMMENT	VARCHAR(1200)	COMMENT'评论内容',	
UID	VARCHAR(18)	COMMENT'用户唯一号',	
FROM_ACCOUNT	VARCHAR(120)	COMMENT'影评人',	
CREATE_DATE	BIGINT	COMMENT'评论收集时间',	
NICE_COUNT	INT	COMMENT'评论点赞数',	
STATUS	VARCHAR(3)	COMMENT'状态	20:待审核状态;30:审核通过;40:审核不通过; 50:已删除'
)COMMENT '视频信息-评论收集';
#7.10 视信息-评论收集点赞（T_V_VIDEO_COLLECT_NICE）
CREATE TABLE IF NOT EXISTS T_V_VIDEO_COLLECT_NICE (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
VID	INT	COMMENT'电影编号',	
UID	VARCHAR(18)	COMMENT'用户编号',	
CREATE_DATE	BIGINT	COMMENT'操作日期',	
...
#7.11 演员照片墙表(T_V_PHOTO_WALL)
CREATE TABLE IF NOT EXISTS T_V_PHOTO_WALL (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
PHOTO	VARCHAR(180)	COMMENT'照片URL'
)COMMENT '演员照片墙表';
#7.12 演员-演员照片墙表(T_V_PERFORMER_PHOTO_WALL)
CREATE TABLE IF NOT EXISTS T_V_PERFORMER_PHOTO_WALL (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
PID	INT	COMMENT'演员编号',
PHOTO_ID	VARCHAR(180)	COMMENT'照片编号'
)COMMENT '演员-演员照片墙表';
#7.13地区表(T_V_AREA)
CREATE TABLE IF NOT EXISTS T_V_AREA (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
AREA	VARCHAR(88)	COMMENT'地区l',
NATIONALITY	VARCHAR(88)	COMMENT'国籍'
)COMMENT '地区表';
#网站访问信息统计表
CREATE TABLE IF NOT EXISTS T_D_WEBSITE_visit_info (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
PV  INT COMMENT'浏览量',
VV  INT COMMENT'访问次数',
IP  INT COMMENT'独立ip访问量',
UV  INT COMMENT'独立访客',
COUNT_DATE BIGINT COMMENT'統計数据日期'
)COMMENT '网站访问信息统计表';
#网站独立IP访问记录表
CREATE TABLE IF NOT EXISTS T_D_WEBSITE_ip_info (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
IP_ADDR VARCHAR(15)COMMENT'访问的IP地址',
IP_VISIT_TIME BIGINT COMMENT'IP访问的时间',
CREATE_DATE BIGINT COMMENT'创建时间'
)COMMENT '网站访独立IP访问记录';
#日志独立IP访问记录
CREATE TABLE IF NOT EXISTS t_w_work_ip_info (
ID	INT PRIMARY KEY AUTO_INCREMENT COMMENT'编号',
IP_ADDR VARCHAR(15)COMMENT'访问的IP地址',
IP_VISIT_TIME BIGINT COMMENT'IP访问时间',
TID INT COMMENT'文章Id'
CREATE_DATE BIGINT COMMENT'创建时间'
)COMMENT '日志独立IP访问记录';
